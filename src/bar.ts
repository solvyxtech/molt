...
export function claimedCreated(claim: string, mentioned: string[]): string[] {
  ...
  for (const path of mentioned) {
    ...
    const asserted = [inBackticks, backtickFirst, prose].some((re) =>
      [...claim.matchAll(new RegExp(re.source, "gi"))].some((m) => !disclaimed(claim, m.index!, m[0])),
    );
    if (asserted && !out.includes(bare)) out.push(bare);
  }
  return out;
}

/**
 * Is this match the claim disclaiming the creation rather than making it?
 *
 * A live run was refused three attempts in, on correct work, for writing "I
 * overstated by describing `coverage/lcov.info` as something I created — it's
 * build output … not a file I authored". Naming a file in order to say you
 * did NOT make it is the opposite of the fabrication this check exists for.
 * Two readings excuse a match, and nothing else does:
 *
 *  - a negation before the verb, or inside the match ("did not create
 *    `x`", "`x` was never added") — not one elsewhere in the sentence, so
 *    "created `x`, which did not exist before" is still a creation claim;
 *  - the same sentence crediting the file to a build or a command ("build
 *    output", "generated by", "as a side effect").
 */
function disclaimed(claim: string, at: number, match: string): boolean {
  // Short: a negation of THIS verb sits against it ("did not create"). A wider
  // window let "I didn't just fix it, I created `x`" disclaim a real claim.
  const before = claim.slice(Math.max(0, at - 12), at);
  if (/\b(?:not|never|no longer|instead of)\b|n['’]t\b/i.test(before + match)) return true;
  const start = Math.max(claim.lastIndexOf("\n", at), claim.slice(0, at).search(/[.!?]\s[^.!?]*$/)) + 1;
  const endRel = claim.slice(at + match.length).search(/[.!?](?:\s|$)|\n/);
  const sentence = claim.slice(start, endRel < 0 ? claim.length : at + match.length + endRel);
  return /\b(?:build output|generated by|written by|output of|side effect|is generated|gets generated)\b/i.test(sentence);
}
