...
export function parseArgs(argv: string[]): Args {
  const out: Args = {
    cmd: "",
    url: process.env.MOLT_BASE_URL ?? "http://localhost:11434/v1",
    model: process.env.MOLT_MODEL ?? "qwen2.5-coder:7b",
    key: process.env.MOLT_API_KEY,
    cwd: process.cwd(),
    explain: false,
    raw: false,
    stream: true,
...
function buildEngine(args: Args): Engine {
  ...
  journal.append("session_start", {
    ...
    molt: VERSION,
    provider: args.provider ?? new URL(args.url).hostname.split(".")[0],
    model: args.model,
    endpoint: args.url,
    cwd: args.cwd,
  ...
  const emit = (ev: EngineEvent) => {
    ...
    switch (ev.kind) {
      ...
      case "assistant_text":
        ...
        break;
      case "tool":
        process.stdout.write(`· ${ev.name}  ${ev.detail}${ev.note ? `  [${ev.note}]` : ""}\n`);
        break;
      case "proof_start":
