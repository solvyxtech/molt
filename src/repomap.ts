 ...
 *
 * Built output and tool scratch space are not source, and mapping them is
 ...
 * and a worktree checkout's `src/engine.ts`: the same files two and three
 * times over, crowding real ones out of the budget and each copy standing as
 * evidence that the others are important.
 *
 ...
 * added by hand. Hidden directories go the same way: a tool's worktrees,
 * hooks or caches live under a dot-name, and none of them are the source.
 */
export function mapSkipDirs(cwd: string): Set<string> {
  const skip = new Set<string>([...SEARCH_SKIP_DIRS, ".husky", "__snapshots__"]);
  try {
    for (const e of readdirSync(cwd, { withFileTypes: true })) {
      if (e.isDirectory() && (/^dist/i.test(e.name) || e.name.startsWith("."))) skip.add(e.name);
    }
  } catch {
