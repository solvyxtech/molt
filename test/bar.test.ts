...
describe("claimedCreated", () => {
  ...
  it("does not read a disclaimer as the claim it disclaims", () => {
    // The exact claim a live Sonnet run was refused on, three attempts in,
    // for correct work: it named the file only to say it had NOT made it.
    const claim =
      'I overstated by describing `coverage/lcov.info` as something I "generated"/"created" as ' +
      "work product — it's build output from running the test command, not a file I authored. " +
      "I won't claim credit for it; `npm test` writes it as a side effect.";
    assert.deepEqual(claimedCreated(claim, ["coverage/lcov.info"]), []);
    assert.deepEqual(claimedCreated("I did not create `src/a.ts`; it already existed.", ["src/a.ts"]), []);
    assert.deepEqual(claimedCreated("`dist/x.js` is generated by the build.", ["dist/x.js"]), []);
  });

  it("still flags a creation claim that sits beside an unrelated negative", () => {
    assert.deepEqual(
      claimedCreated("Created `src/new.ts`, which did not exist before.", ["src/new.ts"]),
      ["src/new.ts"],
      "a negation about something else is not a disclaimer of this",
    );
    assert.deepEqual(
      claimedCreated("I didn't just fix it, I created `src/new.ts`.", ["src/new.ts"]),
      ["src/new.ts"],
    );
  });
});
