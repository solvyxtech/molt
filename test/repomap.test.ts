...
describe("building one for a real directory", () => {
  ...
  it("does not map the same file three times over", async () => {
    ...
    // `dist-test/src/line.js` and `.worktrees/agent-a03f…/src/line.ts` — copies
    // of one file, crowding real ones out of the budget, each copy standing as
    // evidence that the others mattered.
    const dir = ws();
    for (const sub of ["src", "dist-test/src", ".worktrees/agent-1/src"]) {
    ...
    assert.ok(skip.has(".worktrees"), "a tool's worktrees are whole copies of the repository");
